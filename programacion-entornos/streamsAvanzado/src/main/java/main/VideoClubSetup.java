...
public class VideoClubSetup {

  public void setUpVideoClub(){
    setupSocioSocios();
    setupProductos();
    setAlquileres();
  }
  ...
  private static void setupSocioSocios() {
    ...
    for (int i = 0; i < 100; i++) {
      Boolean sancionado = random.nextBoolean();
      Socio socio = new Socio(f.phoneNumber().extension(), f.name().firstName(), f.gameOfThrones().city(), f.phoneNumber().subscriberNumber(), f.number().numberBetween(1, 99), sancionado);
      sv.addSocio(socio);
    }
  }
  ...
  private static void setupProductos() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      pelicula.setCantidadAlquilada(r.nextInt(100));
      pelicula.setValoracionMedia(r.nextDouble()*5);
      sv.addProducto(pelicula);
    }
    for (int i = 0; i < 100; i++) {

      Producto videoJuego = new Videojuego(f.name().title(), r.nextInt(10), f.music().genre(), f.name().lastName());
      videoJuego.setCantidadAlquilada(r.nextInt(100));
      videoJuego.setValoracionMedia(r.nextDouble()*5);
    ...
    for (int i = 0; i < 100; i++) {
      ...
      documental.setCantidadAlquilada(r.nextInt(100));
      documental.setValoracionMedia(r.nextDouble()*5);
